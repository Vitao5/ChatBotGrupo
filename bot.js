...
async function processarMidiaComoSticker(sock, from, msg) {
  ...
  try {
    ...
    await sock.sendMessage(from, {
      text: "Falhei na conversão ( ﾉ ﾟｰﾟ)ﾉ, tenta ai novamente",
    });
  }
}
...
async function verificaAgendaAva(marcarPessoasGrupo, sock, from) {
  ...
  const semEventos =
    !eventosHoje && !eventosAmanha && !eventos2dias && !eventos3dias;
...
async function start() {
  ...
  sock.ev.on("messages.upsert", async ({ messages }) => {
    ...
    if (
      temImagem &&
      (caption === "!figurinha" || caption.includes("!figurinha"))
    ) {
